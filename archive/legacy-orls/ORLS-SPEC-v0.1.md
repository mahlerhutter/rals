# Open Renewables Listing Standard (ORLS)
...
The Open Renewables Listing Standard (ORLS) defines a machine-readable, vendor-neutral format for describing renewable energy assets that are offered for sale, refinancing, or partnership. It enables sellers, advisors, and prospective buyers — including AI agents acting on behalf of institutional investors — to discover, parse, and evaluate utility-scale renewable assets without proprietary data rooms or bespoke spreadsheet templates.

ORLS is a YAML 1.2 serialization format hosted at well-known URLs on seller or advisor domains, with a two-tier access model: a public **Teaser Profile** discoverable by any agent, and an authenticated **Full Profile** accessible after NDA via OAuth 2.0 token exchange.
...
Comments are welcomed via GitHub Issues at `github.com/orls-spec/spec`.

The editor expects breaking changes between v0.1 and v1.0. Implementations of v0.1 SHOULD declare `orls_version: "0.1"` and MUST be prepared to migrate.
...
- **Teaser Profile** refers to the publicly accessible subset of an ORLS document.
- **Full Profile** refers to the complete ORLS document, accessible only after authentication.

---

## 3. Design Principles

ORLS is designed around five principles. Implementers SHOULD understand them before extending or critiquing the standard.

**P1 — Machine-first, human-readable.** YAML was chosen over JSON-LD or XML because deal teams maintain ORLS documents in version control alongside their data rooms. Humans must be able to read and diff them.

**P2 — Two-tier confidentiality is intrinsic.** Unlike property listings, renewable asset transactions are confidential by default. ORLS does not pretend otherwise. Every field is classified as `public`, `nda_required`, or `final_shortlist`.

**P3 — Bring-your-own-data-room.** ORLS does not replace Drooms, Datasite, or Intralinks. It references them. Sellers continue to operate their data rooms; ORLS provides the structured index.

**P4 — Opinionated where the industry isn't.** Where industry practice has settled (e.g. P50/P75/P90 production estimates), ORLS adopts it. Where practice is fragmented (e.g. OPEX line-item structure), ORLS imposes a canonical taxonomy and lets implementers extend it.
...
An ORLS document MUST be a valid YAML 1.2 file, UTF-8 encoded, with file extension `.orls.yaml` or hosted at the path `/.well-known/orls.yaml`.
...
```
https://seller.example.com/.well-known/orls.yaml
```

For sellers offering multiple assets, a directory index MAY be served at `/.well-known/orls/index.json`:
```json
{
  "version": "0.1",
  "assets": [
    "/.well-known/orls/asset-001.yaml",
    "/.well-known/orls/asset-002.yaml"
  ],
...
```html
<link rel="orls" href="/assets/operating-pv-iberia.orls.yaml" type="application/yaml" />
```

**Method 3 — Registry Submission:**
Sellers MAY submit their well-known URI to the public ORLS Registry (operated by the ORLS Working Group), which provides a queryable index for buyer agents. Registry submission is OPTIONAL.

### 4.3 Confidentiality Tiers

Every field in the ORLS schema carries a confidentiality tier:
...
An ORLS document consists of seven top-level sections plus a metadata header:

```yaml
orls_version: "0.1"
document_id: "orls:01H8XY...PVN9"
generated_at: "2026-05-15T10:00:00Z"
generated_by: "seller.example.com/orls-generator/v1.2"
language: "en"
...
| `access_endpoint` | URL | SHOULD | public | ORLS-compliant authentication endpoint (see 12.5). |
...
The `data_room` object carries `provider`, `url`, `access_mechanism`, an approximate `document_count_approx`, a `structure` note, and a `vendor_dd_reports` list. The data room itself remains operated by the seller's chosen provider; ORLS only indexes it (Design Principle P3).

### 12.5 Access Endpoint

For programmatic access to the Full Profile, ORLS defines an OAuth 2.0–compatible flow:

```
POST https://seller.example.com/.well-known/orls/auth

Request:
{
  "asset_id": "orls:01H8XY...PVN9",
...
A conforming ORLS document MUST pass validation against the JSON Schema published at `https://orls.org/schema/v0.1/orls.schema.json`.
...
A document declaring `orls_version: "0.1"` MUST achieve at least Level 1.
...
ORLS documents at Level 2 or 3 contain commercially sensitive information. Sellers MUST NOT host Full Profiles at publicly accessible URLs without authentication. The well-known URI serves the **Teaser Profile only**. Full Profiles MUST be behind the access endpoint described in Section 12.5 or equivalent.
...
- **Multi-document portfolio aggregation** — how a portfolio ORLS references its constituent single-asset files.
...
Complete fictional ORLS documents are maintained as separate files rather than inlined in this specification, so that the examples and the normative text cannot drift apart:

- **`examples/project-helios.orls.yaml`** — Project Helios, an operating 87.5 MW (AC) / 105 MW (DC) solar PV plant in Castilla-La Mancha, Spain. Level 3 (Complete). Exercises the PV-specific `technical` fields (`module`, `inverter`, `mounting`), a hybrid corporate-PPA + CfD revenue stack, and Spanish REER subsidy mechanics.
- **`examples/project-aeolus.orls.yaml`** — Project Aeolus, an operating 148.5 MW onshore wind farm in Schleswig-Holstein, Germany. Level 3 (Complete). Exercises the wind-specific `technical` fields (`turbine_configurations`, `wind_resource`, `noise_emission`, `shadow_flicker`), the EEG gleitende Marktprämie revenue mechanism, BImSchG/BNatSchG permitting, and repowering optionality.
...
Dieser Annex dokumentiert die Verknüpfung von ORLS-Feldern mit deutschen, österreichischen und EU-rechtlichen Anforderungen. Er ist informativ, nicht normativ.
...
Käufer, die als SFDR Article 8 oder Article 9 Fonds operieren, benötigen die Felder unter `compliance.eu_taxonomy_alignment` vollständig ausgefüllt. ORLS-Implementierungen für Verkäufer SOLLEN diese Felder als technisch verpflichtend behandeln, wenn der Käuferkreis institutionelle EU-Investoren umfasst.
...
The ORLS document is not a replacement for your Information Memorandum. It is a structured layer beneath it. Generate the ORLS file from the same source data you use for your IM and data room. A well-maintained ORLS file reduces buyer Q&A volume by an estimated 30–50% because buyer agents can answer their own questions directly from the structured data.

### C.2 For Advisors

ORLS adoption signals process sophistication. Buyers increasingly run preliminary screening through LLM-based agents; assets without structured data require manual extraction, which delays evaluation and disadvantages your client in competitive processes. Consider offering ORLS generation as a value-added service in mandate proposals.

### C.3 For Buyer Agents

When fetching an ORLS document, MUST honor `confidentiality` tags. Do not include `nda_required` or `final_shortlist` content in summaries or recommendations distributed beyond the authorized buyer team. Audit logging of agent access is RECOMMENDED.
...
ORLS is not endorsed by any industry association at the time of v0.1 publication. Endorsement conversations are open.
...
*End of ORLS Specification v0.1*
